...
function drawContacts() {
  ...
  contacts.forEach(contact => {
    ...
    <div class="card mt-1 mb-1 ${contact.emergencyContact ? `emergency-contact` : ''}">
      ...
      <i type="button" class="action fa fa-trash text-danger" onclick="removeContact('${contact.id}')"></i>
      </div>
    </div>
    `
  })
...
function removeContact(contactId) {
  let remIndex = contacts.findIndex(contact => contact.id == contactId)
  contacts.splice(remIndex,1)
  saveContacts()
}
